...
def run_diayn_MPE():
    params = get_params_DIAYN_MPE() # Edit in Common > config.py

    test_env = simple_tag_v3.env(num_good=2, num_adversaries=4, num_obstacles=2, max_cycles=500, continuous_actions=False, render_mode='human')
    print(test_env)

    ado_obs_space = test_env.observation_spaces['adversary_0'].shape
    ego_obs_space = test_env.observation_spaces['agent_0'].shape

    n_actions = test_env.action_spaces['adversary_0'].n # same for ado and ego
    ...
    env = simple_tag_v3.env(num_good=2, num_adversaries=4, num_obstacles=2, max_cycles=500,
